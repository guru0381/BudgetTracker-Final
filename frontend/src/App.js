...
function App() {
  ...
  const handleRefresh = () => {
    ...
    const newIntervalId = setInterval(() => {
      console.log("CHECKING222", newExpirationTime, Date.now());
      const remainingTime = newExpirationTime - Date.now();

      axios.post(`http://159.203.127.58:3002/refresh-token/${userId}`).then((res) => {
  ...
  return (
    <AuthProvider>
      <Router>
        ...
        {showPopup && (
          <TokenExpirationPopup onRefresh={handleRefresh} onClose={handleLogout} />
        )}
        <Footer />
      </Router>
    </AuthProvider>
